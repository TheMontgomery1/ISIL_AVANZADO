...
public class Goblin_Behaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (!getDamage) 
        { 
            rb_goblin.linearVelocity = new Vector2(movementX * speed, rb_goblin.linearVelocity.y);
        }
            anim_goblin.SetBool("move_gob", Mathf.Abs(movementX) > 0.01f);
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player_sword"))
        {
            ...
            GetComponent<gob_SFX>().PlayGobHit();
        }
    }
